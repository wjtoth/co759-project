...
from .wideresnet_szagoruyko.utils import conv_params, linear_params, bnparams, bnstats, \
        flatten_params, flatten_stats


def resnet(depth, width, num_classes, nonlin, useBN=True):
    assert (depth - 4) % 6 == 0, 'depth should be 6n+4'
    n = (depth - 4) // 6
    widths = torch.Tensor([16, 32, 64]).mul(width).int()

    assert useBN, 'batchnorm is currently required for (wide) resnet'
    ...
    def activation(x, params, stats, base, mode):
        return nonlin(F.batch_norm(x, weight=params[base + '.weight'],
