...
# -- modified to support arbitrary non-linearities and remove batchnorm
...
#        out = self.dense3(#out)
###        #out = torch.squeeze(F.avg_pool2d(self.nonlin(self.bn1(out) if self.useBN else out), 8)) # BAD ORDERING OF POOL & NONLIN
###        out = torch.squeeze(F.max_pool2d(self.nonlin(self.bn1(out) if self.useBN else out), 8))
